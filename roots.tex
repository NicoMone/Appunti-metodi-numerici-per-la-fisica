...
Applicando l'iterazione di Newton di eq.(\ref{Newton-Raphson}) si ottiene per l'errore:
...
\subsection{Osservazioni sul metodo di Newton}
...
\begin{multicols}{2}
\begin{itemize}
	\item richiede una buona stima iniziale;
...
Si ottiene quindi $G\,=\, \frac{1}{a} + \frac{n-1}{b}$ e $H\,=\, \frac{1}{a^2} + \frac{n-1}{b^2}$:
...
\section{Generalizzazione a più dimensioni}

Il problema della ricerca degli zeri si estende naturalmente a più dimensioni.

Si considerano sistemi del tipo $f_i(\mathbf{X}) = 0$, dove \( \mathbf{X} \) è un vettore in \( \mathbb{R}^N \).

Si generalizza il metodo di Newton-Cotes scrivendo

\[
f_i(\mathbf{X} + \delta)\ \approx\ f_i(\mathbf{X})\, +\, \sum_j\, \frac{\partial f_i}{\partial x_j}\, \delta_j.
\]

Definendo $a_{ij} = \frac{\partial f_i}{\partial x_j}$, si ottiene il sistema lineare:
