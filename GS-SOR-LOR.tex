...
\subsubsection{Tasso di convergenza}

Nel caso del metodo di Gauss-Seidel, il tasso asintotico di convergenza è data da:

\[
R_{\infty}^{GS}\ =\ -\,\ln \rho(\mathbf{H}_{GS})\ =\ -\,2\, \ln \rho(\mathbf{H}_J)\ =\ 2\, R_{\infty}^{J}, 
\]

dove \(R_{\infty}^{J}\) è il tasso asintotico di convergenza del metodo di Jacobi.
...
\[
\phi_{i,\,j}^{(m+1)}\ =\ W_{i,\,j}\, \phi_{i-1,\,j}^{(m+1)}\, +\, E_{i,\,j}\, \phi_{i+1,\,j}^{(m)}\, +\, S_{i,\,j}\, \phi_{i,\,j-1}^{(m+1)}\, +\, N_{i,\,j}\, \phi_{i,\,j+1}^{(m)}\, +\, q_{i,\,j}\ \ . 
\]
...
Dalle equazioni precedenti è possibile ricavare la matrice di iterazione dello schema iterativo SOR:
...
Consideriamo ora il seguente limite: \[\lim_{m \to \infty} \frac{\left\| \mathbf{H}_J^{m+1} \mathbf{q} \right\|^2}{\left\| \mathbf{H}_J^m \mathbf{q} \right\|^2}\ \ .\]  

Dall'Equazione di $\|\mathbf H^m_j \mathbf q\|^2$, per \(m \to \infty\), i rapporti nella sommatoria per \(h: |\lambda_h| < \rho_{\mathbf{H}_J}\) tenderanno tutti a zero, così che il limite si riduce a:

\[
\lim_{m \to \infty} \frac{\left\| \mathbf{H}_J^{m+1} \mathbf{q} \right\|^2}{\left\| \mathbf{H}_J^m \mathbf{q} \right\|^2}\ =\
\lim_{m \to \infty} \frac{\rho_{\mathbf{H}_J}^{2m+2}}{\rho_{\mathbf{H}_J}^{2m}} \cancel{\frac{\sum_{h: |\lambda_h| = \rho_{\mathbf{H}_J}} |q_h|^2}{\sum_{h: |\lambda_h| = \rho_{\mathbf{H}_J}} |q_h|^2}}\ =\ \rho_{\mathbf{H}_J}^2\ \ . 
\]
...
si ha: \[\left\| \mathbf{\phi}^{(m+1)} - \mathbf{\phi}^{(m)} \right\| = \left\| \mathbf{H}_J^m \mathbf{q} \right\|\ \  .\]

Dall'equazione di iterazione, segue che:
...
\begin{multicols}{2}
Pertanto, vengono eseguite diverse iterazioni di Jacobi fino a quando il rapporto \(\displaystyle\frac{\left\| \mathbf{\phi}^{(m+2)} - \mathbf{\phi}^{(m+1)} \right\|}{\left\| \mathbf{\phi}^{(m+1)} - \mathbf{\phi}^{(m)} \right\|}\) non viene considerato una stima sufficientemente accurata del raggio spettrale.

Inoltre si può mostrare che $\omega=\omega_{opt}$:
\[\rho_{\mathbf H_{SOR}}\ =\ \frac{2}{1+\sqrt{1-\rho^2_{\mathbf H_J}}}\,-\,1\]

\begin{figure}[H]
	\includegraphics[width=8cm]{img/gs3.png}
	\label{SOR}
	\caption{Gauss-Siedel e Successive OverRelaxation comparati al raggio di convergenza di Jacobi.}
\end{figure}
\end{multicols}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=8cm]{img/gs4.png}
	\label{red}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=8cm]{img/gs5.png}
	\label{and black}
...
L'equazione generica del sistema, scritta in forma a blocchi, è:
...
\[
\overset{\text{line to update}}{\hat{\mathbf{A}}_{i,i} \hat{\mathbf{\phi}}_i^{(m+1)}}\ =\ \hat{\mathbf{s}}_i\, -\, \overset{\text{line already updated}}{\hat{\mathbf{A}}_{i,i-1} \hat{\mathbf{\phi}}_{i-1}^{(m+1)}}\, -\, \overset{\text{line not yet updated}}{\hat{\mathbf{A}}_{i,i+1} \hat{\mathbf{\phi}}_{i+1}^{(m)}}\ \ . 
\]

L'equazione sopra è un \underline{sistema tridiagonale} che può essere risolto efficientemente con l'algoritmo di Thomas.
...
\subsubsection{Implementazione pratica del metodo LOR}

\begin{figure}[H]
	\centering
	\includegraphics[width=10cm]{img/gs6.png}
	\label{LOR}
	\caption{implementazione LOR}
\end{figure}
...
È un'equazione a tre punti. 

Si noti che i coefficienti $W$, $E$, $S$ e $N$ non sono quelli dei metodi precedenti, qui stiamo parlando della matrice $\mathbf A$ più che $\mathbf H$. Nonostante il metodo si chiami "line overrelaxation", applicarlo per colonne o alternare fra righe e colonne funziona ugualmente. 

Procedendo per colonne, l'equazione sopra diventa:
